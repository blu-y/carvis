...
class YOLOP:
    def __init__(self, ros=False, device='cpu', img_size=640, topic='/camera/color/image_raw'):
        self.tf = tt()
        ...
        if ros != False:
            ...
            self.sub = rospy.Subscriber(topic, Image, self.cb, 1)
    ...
    def lines(self, img):
        ...
        return img
    def mask2line(img, min, max):
        img_l = np.copy(img)
        [y, x, _] = img_l.shape
        y = int(y/4); x = int(x/4)
        img_l = cv2.resize(img_l, (x, y))
        img_c = cv2.Canny(img_l, 75, 150)
        line = cv2.HoughLinesP(img_c, 1, np.pi/360, 50, minLineLength=min, maxLineGap=max)
        if line is not None:
            print(len(line))
            for l in line:
                [x1, y1, x2, y2] = l[0]
    ...
    cv2.createTrackbar('minLineLength', 'line', 0, 1000, nothing)
    cv2.createTrackbar('maxLineGap', 'line', 0, 1000, nothing)
    cv2.setTrackbarPos('minLineLength', 'line', 10)
    cv2.setTrackbarPos('maxLineGap', 'line', 10)
    img = cv2.imread('mask.png')
    while True:
        m = cv2.getTrackbarPos('minLineLength', 'line')
        M = cv2.getTrackbarPos('maxLineGap', 'line')
        img_l, line = mask2line(img, m, M)
...
#cap = cv2.VideoCapture('IMG_0861.mp4')
#'/home/vis/kcity_driving_rosbag/first/_2022-09-18-13-38-45_0.bag'
...
def camera_cb(data):
    ...
    img_det, ll_img = yolop.detect(img)
    cv2.imshow('img', img_det)
    cv2.imshow('ll)mask', ll_img)
    cv2.waitKey(1)
rospy.init_node('yolop')
camera_sub = rospy.Subscriber('/camera/color/image_raw', CompressedImage, camera_cb, queue_size = 1)
rate = rospy.Rate(1)
