...
def edges(masked):
    ...
    lines = cv2.HoughLinesP(edges, 1, np.pi/180, threshold=170, maxLineGap=50)
    return lines
...
def draw_line(lines, slope, masked, flag): 
...
def detect_line(slope, lines):
    ...
    # print(angle)
    # print('2 ', slope)
    if (-1< np.any(slope)<10 or -10<np.any(slope)<1):  # 정지선인지 확인하기, 정지선일 가능성이 있는 직선 존재 확인
...
cap2 = cv2.VideoCapture(openCvVidCapIds[0])
cap = cv2.VideoCapture(openCvVidCapIds[1])
