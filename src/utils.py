...
class pathReader :  ## 텍스트 파일에서 경로를 출력 ##
    ...
    def read_txt(self,file_name):
        ...
        #     out_path.poses.append(read_pose)
            out_path.append([float(tmp[0]), float(tmp[1])])
...
class purePursuit_track : ## purePursuit 알고리즘 적용 ##
    def __init__(self):
    ...
    def getPath(self,x_ref, y_ref):
        self.x_ref=x_ref 
        self.y_ref=y_ref
    
    def getEgoStatus(self,velocity,my_heading):

        self.current_vel=velocity  #kph
        self.vehicle_yaw=my_heading/180*pi   # rad
        self.cur_x=0 ## 차량의 현재x 좌표 ##
        self.cur_y=0 ## 차량의 현재y 좌표 ##
    ...
    def steering_angle(self): ## purePursuit 알고리즘을 이용한 Steering 계산 ## 
        ...
        dx= self.x_ref - self.cur_x
        dy= self.y_ref - self.cur_y
...
class purePursuit : ## purePursuit 알고리즘 적용 ##
    ...
    def getEgoStatus(self,msg,velocity):

        self.current_vel=velocity  #kph
        self.vehicle_yaw=msg.z  # rad
        self.current_postion.x=msg.x ## 차량의 현재x 좌표 ##
        self.current_postion.y=msg.y ## 차량의 현재y 좌표 ##
...
class vaildObject : ## 장애물 유무 확인 (차량, 사람, 정지선 신호) ##
    ...
    def get_object(self,num_of_objects,pose_x,pose_y):
    ...
    def calc_vaild_obj(self,ego_pose):  # x, y, heading
        ...
        for num in range(self.all_object.num_of_objects):
            ...
            if local_result[0][0]> 0 :
                global_object_info.append([self.all_object.pose_x[num],self.all_object.pose_y[num]])
                loal_object_info.append([local_result[0][0],local_result[1][0]])
...
class pidController : ## 속도 제어를 위한 PID 적용 ##
    ...
    def pid(self,target_vel,current_vel,status_msg):
        error= target_vel-current_vel * cos(status_msg.z)
