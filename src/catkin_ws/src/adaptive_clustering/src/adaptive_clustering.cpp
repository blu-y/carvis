...
ros::Publisher center_array_pub_;
...
void pointCloudCallback(const sensor_msgs::PointCloud2::ConstPtr& ros_pc2_in) {
  ...
  for(int i = 0; i < pc_indices->size(); i++) {
	...
	pcl_pc_in->points[(*pc_indices)[i]].z * pcl_pc_in->points[(*pc_indices)[i]].z;
      float x2 = pcl_pc_in->points[(*pc_indices)[i]].x;
      float y2 = pcl_pc_in->points[(*pc_indices)[i]].y;

	if(d2 > range * range && d2 <= (range+regions_[j]) * (range+regions_[j]) && x2 < 5 && x2 > 0 && y2 < 0 && y2 > -1.5) {
  ...
  center_x /= clusters.size();
  center_y /= clusters.size();
  center_z /= clusters.size();
  if(center_array_pub_.getNumSubscribers() > 0) {
    Eigen::Vector4f centroid;
    // pcl::compute3DCentroid(*clusters[i], centroid);
    ...
    pose.orientation.w = 1;
    center_array.poses.push_back(pose);
  ...
  if(center_array.poses.size()) {
    center_array.header = ros_pc2_in->header;
    center_array_pub_.publish(center_array);
  }  
...
int main(int argc, char **argv) {
  ...
  center_array_pub_ = private_nh.advertise<geometry_msgs::PoseArray>("center", 100);
